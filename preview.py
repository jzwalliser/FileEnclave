...
def preview(file_path,quality=100,max_width=960,max_height=540):
    if pathlib.Path(file_path).suffix in ".".join(["md","jpeg","jpg","jpe","jfif","png","webp","tiff","tif","jp2","jpx","avif","gif","bmp","mp4","mov","avi","mkv","webm","flv","ts","mpg"]):
        try:
            return add_background(preview_media(file_path,max_width=960,max_height=540),width=960,height=540)
        except:
            traceback.print_exc()
            return add_background(PIL.ImagePaletteImage.new("RGB",(1,1), (0, 0, 0)))
