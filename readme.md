# 简介
这是文件加密工具，可用于加密各式各样的文件。有不少Vibe-coding的成分；其中有30%的代码是元宝写的（在此非常感谢元宝），剩下70%代码是我写的，几乎所有测试、修bug工作是我做的。
...
2. 接着，在内部生成随机`salt`，并使用安全系数极高的`argon2id`算法对用户密钥进行派生，派生密钥记为`user_password`。
3. 然后，再次在内部生成一个随机密码，记为`file_password`。
4. 之后，创建7z压缩包，用`user_password`将`file_password`加密存储到压缩包中。
5. 对于需要加密的文件，首先为其生成预览图，接着将其切片，推荐的切片大小为`10 MB`。
6. 对于每个切片，使用`file_password`将其加密压缩，同时也将预览图加密压缩。
...
9. 当用户想要打开文件的时候，会先解密，然后挂载一个`fuse`文件系统，并将解密后的文件放入其中。
...
2. 我使用了`argon`算法，对用户密钥进行了派生，这样即使7z压缩包不慎流露了出去，也非常难被破解，即使用户设置的密码非常简单也因为密码派生的原因变得很复杂。
3. 文件的加密密码是内部的一个随机数`file_password`，这样如果用户想改密码，就不需要把数据全部解压出来再重新写回去了，大大提高了效率。
4. 使用`fuse`文件系统，这样全程数据不落盘，全部存在于内存中，卸载文件系统之后，文件无法恢复；如果这时候电脑突然关机，文件立即被抹去，不发生文件残留在硬盘中、来不及删掉的事故（但是有可能残留在`swap`分区）。
