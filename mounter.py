...
class SevenZipMemoryFuse(fuse.Operations):
    def __init__(self,mountpoint,archive,password,mount_filename=None,max_cache_size=20 * 1024 ** 2,open_file=False):
        ...
        meta = self.load_json("metadata") #元数据
        salt = meta["salt"]
        ...
        self.password = self.read_file("password",password=user_password).decode("utf-8")
        print(f"Loaded real password: {self.password}")
        self.chunks = meta["chunks"] #切块数量
        self.chunk_size = meta["chunk_size"] #切块大小
        original_meta = self.load_json("original_metadata",password=user_password) #加密元数据
    ...
    def load_json(self,name,password=None):
        content = self.read_file(name,password)
        return json.loads(content) #读取配置

    def get_chunk(self,index): #获取切块
        ...
        chunk = self.read_file(str(index),password=self.password) #未缓存切块，从压缩包中读取
    ...
    def read(self,path,size,offset,fh): #读取文件
        ...
        while len(result) < size and offset < self.size: #返回值未到达指定大小且文件没有读完
            ...
            chunk = self.get_chunk(index) #获取切块数据
