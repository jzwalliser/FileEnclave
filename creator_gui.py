...
def run_creator(): #创建加密压缩包
    ...
    cmd = ["python3","creator.py",file_path,f"{os.path.join(output_entry.get(),output_hash)}.7z",user_password_var.get(),"-y","-m",f'{{"tags":[{tags_json}],"folder":"{virtual_directory_var.get()}"}}',"-c",str(calc_size(chunk_size_entry.current(),chunk_size_entry.get()))]
    print(cmd)
...
main_pane.add(right_frame,minsize=820)
...
tag_entry_frame = tkinter.Frame(frame_tags)
tag_entry_frame.pack(fill=tkinter.X)
tag_entry = tkinter.Entry(tag_entry_frame,textvariable=tags_var,width=30)
tag_entry.pack(side=tkinter.LEFT,fill=tkinter.X,expand=True)
tag_reset = ttkbootstrap.Button(tag_entry_frame,text="×",command=lambda: tags_var.set(value=""),cursor="arrow",bootstyle=ttkbootstrap.constants.LINK)
tag_reset.pack(side=tkinter.LEFT)
...
parser.add_argument("-o","--output",help="Output directory",default=path)
...
output_entry.insert(tkinter.INSERT,args.output)
